...
def create_bridge(factory, msg_type, topic_from, topic_to, **kwargs):
    ...
    if not issubclass(msg_type, rospy.Message):
        raise TypeError(
            "msg_type should be rospy.Message instance or its string" "reprensentation"
        )
    return factory(
    ...
    def _create_ros_message(self, mqtt_msg):
        ...
        def remove_null_values(json_data):
            if isinstance(json_data, dict):
                ...
                return [
                    remove_null_values(item) for item in json_data if item is not None
                ]
            else:
                return json_data

        cleaned_json = remove_null_values(json.loads(mqtt_msg.payload))

        msg_dict = self._deserialize(json.dumps(cleaned_json))
